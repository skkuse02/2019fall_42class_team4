  ...
  <v-card height="100%">
    <v-container fluid grid-list-md>
      <v-flex xs12>
        <v-card flat>
          <!-- 상품 사진 -->
          <v-img :src=similarItems[i-1].img_src height="200" contain></v-img>
          <v-card-title primary-title>
            <!-- 상품 이름 -->
            <!-- <router-link :to="{name: 'ItemDetail', params: {id:similarItems[i-1].id}}"><h2>{{similarItems[i-1].name}}</h2></router-link> -->
            <span id="link" @click="pageReload(similarItems[i-1].id)"><h2>{{similarItems[i-1].name}}</h2></span>
          </v-card-title>
          <v-card-text>
            <span style="color: blue;"><h3>Free Delivery</h3></span>
            <!-- 상품 키워드 -->
            <span v-for="keyword in similarItems[i-1].rep_keywords" :key="keyword.keyword">
            ...
            <!-- 상품 별점 -->
            <v-rating
              v-model="similarItems[i-1].scores"
              readonly
              small
